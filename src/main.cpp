...
//     std::string cmd =
//         "./build/importer \"" + csv_path + "\" \"" + out_dir + "\"";
...
//    public:
//     void load(const std::string& dir) { storage_.load_from_dir(dir); }
...
//             for (size_t i = 0; i < out.count; ++i)
//                 nbrs.insert(out.neighbors[i]);
//             auto in = storage_.in_edges(*internal);
//             for (size_t i = 0; i < in.count; ++i)
//                 nbrs.insert(in.neighbors[i]);
...
//     uint64_t shortest_path_exists(std::string_view src_id,
...
//     std::cout << "🚀 Speed: " << static_cast<size_t>(lines / secs)
//               << " rows/s\n";
...
//                     q.edges.push_back({static_cast<size_t>(e["src"].i()),
...
//                     return crow::response(400,
//                                           "{\"error\":\"missing src or dst\"}");
//                 }
//                 uint64_t cnt = service.shortest_path_exists(body["src"].s(),
//                                                             body["dst"].s());
//                 crow::json::wvalue res;
//                 res["count"] = cnt;
//                 return crow::response(crow::json::dump(res));
