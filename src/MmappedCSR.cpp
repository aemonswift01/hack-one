...
MmappedCSR::~MmappedCSR() {
    ...
    if (edge_labels_)
        munmap((void*)edge_labels_, mapped_size_labels_);
}
...
void MmappedCSR::load(const std::string& prefix) {
    ...
    if (fd_offsets == -1)
        throw std::runtime_error("Failed to open " + offsets_path);
    ...
    offsets_ = (const uint32_t*)mmap(nullptr, mapped_size_offsets_, PROT_READ,
                                     MAP_PRIVATE, fd_offsets, 0);
    close(fd_offsets);
    ...
    neighbors_ = (const uint32_t*)mmap(nullptr, mapped_size_neighbors_,
                                       PROT_READ, MAP_PRIVATE, fd_neighbors, 0);
    close(fd_neighbors);
    ...
    edge_labels_ = (const uint8_t*)mmap(nullptr, mapped_size_labels_, PROT_READ,
                                        MAP_PRIVATE, fd_labels, 0);
    close(fd_labels);
